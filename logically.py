...
def create_bets_history_table_if_not_exists():  # შექმნის თეიბლს ცხრილში, თუ არ არსებობს
    ...
    logger.info("The connection to the base is established")
    cursor = conn.cursor()  #
    try:  #
...
def insert_bets(bets_dict):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute(insert_bets_query,
                       [bets_dict['number'], bets_dict['red'], bets_dict['black'], bets_dict['odd'], bets_dict['even'],
                        bets_dict['low'], bets_dict['high'], bets_dict['col1'], bets_dict['col2'], bets_dict['col3'],
                        bets_dict['sec1'], bets_dict['sec2'], bets_dict['sec3']])
        conn.commit()
    finally:
        conn.close()
        cursor.close()
...
def check_red(check_color):
    ...
    for i in range(8, len(check_color)):
        try:
...
def check_odd(check_odd_even):
    global droebiti_cvladi
    for i in range(8, len(check_odd_even)):
        try:
...
def check_low(check_low_high):
    global droebiti_cvladi
    for i in range(8, len(check_low_high)):
        try:
...
def check_col1(check_line):
    global droebiti_cvladi
    for i in range(8, len(check_line)):
        try:
...
def check_sector1(check_sector):
    global droebiti_cvladi
    for i in range(8, len(check_sector)):
        try:
...
def check_sector3(check_sector):
    ...
    for i in range(8, len(check_sector)):
        try:
            if check_sector[0] == "sector3" and "sector3" not in check_sector[1:i + 1] and check_sector[i + 1] == "sector3":
